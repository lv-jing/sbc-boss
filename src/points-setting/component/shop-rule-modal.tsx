...
class ShopRuleModalForm extends React.Component<any, any> {
  ...
  render() {
    ...
    const pointsRate = formData.get('pointsRate');
    const isParentPointsRate = formData.get('isParentPointsRate');
    ...
    return (
      <Form className="login-form">
        ...
        <FormItem {...formItemLayout} label="积分获取比例" hasFeedback>
          {getFieldDecorator('pointsRate', {
            rules: [
              { required: true, message: '请填写获取比例' },
              { pattern: ValidConst.discount, message: '请填写正确的获取比例' }
            ],
            initialValue: pointsRate,
  ...
  _changeCateRate = (e) => {
    ...
    const isParentPointsRate = formData.get('isParentPointsRate');
    editFormData(Map({ pointsRate: e.target.value ? e.target.value : 0 }));
    if (isParentPointsRate == 1) {
      useParentRateF(0);
    }
  };
  ...
  _checkedHandle(checked) {
    ...
    editFormData({ pointsRate: checked ? parentRate : null });
    this.props.form.setFieldsValue({
      pointsRate: checked ? parentRate : null
    });
