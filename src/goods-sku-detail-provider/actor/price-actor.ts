...
export default class PriceActor extends Actor {
  ...
  editPriceSetting(state, { key, value }: { key: string; value: any }) {
    if (key === 'mtkPrice' && state.get('priceOpt') == 0) {
      ...
      userLevelList.toArray().forEach((v) => {
        userLevelPrice = userLevelPrice
          .setIn(
            [v.get('customerLevelId') + '', 'price'],
            QMFloat.accMul(v.get('customerLevelDiscount'), value).toFixed(2)
          )
          .setIn(
      ...
      state = state.set('userLevelPrice', userLevelPrice);
    }
