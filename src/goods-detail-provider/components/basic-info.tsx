...
export default class BasicInfo extends React.Component<any, any> {
  ...
  render() {
    ...
    return (
      <div>
        <GreyBox>
          <p>商家：{storeInfo ? storeInfo.get('supplierName') : '-'}</p>
          <p>商家编码：{storeInfo ? storeInfo.get('supplierCode') : '-'}</p>
          <p>店铺：{storeInfo ? storeInfo.get('storeName') : '-'}</p>
        </GreyBox>
        <div style={{ marginTop: 20 }}>
          ...
          <Form style={{ marginTop: 20 }}>
            ...
            <Row>
              ...
              {/*  <Col span={10}>
                <FormItem {...formItemLayout} label="划线价格">
                  <div>
                    {goods.get('linePrice') == null
                      ? '-'
                      : goods.get('linePrice').toFixed(2)}
                  </div>
                </FormItem>
              </Col>*/}
            </Row>
            ...
            <Row>
              <Col span={10}>
                <FormItem {...formItemLayout} label="商品类型">
                  <div>
                    {goods.get('goodsType') == null
                      ? '-'
                      : goods.get('goodsType') == 0
                      ? '实物商品'
                      : '虚拟商品'}
                  </div>
                </FormItem>
        ...
        {/*商品属性*/}
        <div style={{ marginTop: 20 }}>
          <div className="detailTitle">商品属性</div>
          <Form style={{ marginTop: 20 }}>
            {goodsPropDetails &&
              goodsPropDetails.map((detList) => {
                return (
                  <Row
                    type="flex"
                    justify="start"
                  ...
                  >
                    {detList.map((det) => (
                      <Col
                        span={10}
                        key={`${det.get('propId')}_${det.get('detailId')}`}
                      >
                        <FormItem
                          {...formItemLayout}
                          label={det.get('propName')}
                        >
                          <div>{det.get('detailName')}</div>
                        </FormItem>
                      </Col>
                    ))}
                  </Row>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          >
            ...
            <Column
              ...
              key="supplyPrice"
              render={(supplyPrice) => {
                return supplyPrice == null ? 0.0 : supplyPrice.toFixed(2);
              }}
            />
            <Column
              title="建议零售价"
              ...
              key="retailPrice"
              render={(retailPrice) => {
                return retailPrice == null ? 0.0 : retailPrice.toFixed(2);
              }}
            />
        ...
        <div style={{ marginTop: 20 }}>
          <div className="detailTitle">物流信息</div>
          <Form style={{ marginTop: 20 }}>
            <Row type="flex" justify="start">
              <Col span={10}>
        ...
        <div>
          {/* <div className="detailTitle" style={{ marginBottom: 20 }}>图文详情</div>
          <div
