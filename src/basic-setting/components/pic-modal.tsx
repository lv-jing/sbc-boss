...
import { noop, QMUpload, Const, RCi18n } from 'qmkit';
...
export default class PicModal extends React.Component<any, any> {
      ...
      <Modal
        ...
        title={
          <div style={styles.title}>
            <h4>{RCi18n({ id: 'Setting.Picture library' })}</h4>
            <span style={styles.grey}>
              <span>
                <FormattedMessage id={'Setting.selected'} />
                <strong style={styles.dark}>{clickImgsCount}</strong>
                <FormattedMessage id={'Setting.pictures'} />
              </span>
              <span>
                <FormattedMessage id={'Setting.At most selectable'} />
            ...
            <Col span={3}>
              ...
              >
                <Button size="large" onClick={() => this._handleUploadClick()}>
                  <FormattedMessage id="Setting.Local Upload" />
                </Button>
              </QMUpload>
            </Col>
            <Col span={10}>
              <Form layout="inline">
                <FormItem>
                  <Input
                    placeholder={RCi18n({ id: 'Setting.Please enter content' })}
                    value={imageName}
              ...
              {(imgs || fromJS([])).size > 0 ? null : (
                ...
                >
                  <span>
                    ...
                    <FormattedMessage id="Setting.No data" />
                  </span>
                </div>
              )}
            </Col>
   ...
   */
  _search = () => {
    const { initImg, imageName, cateId, saveSearchName } =
      this.props.relaxProps;
  ...
  _editImages = async (info) => {
    ...
    if (status === 'done') {
      ...
      message.success(
        `${file.name} ${RCi18n({ id: 'Setting.Uploadedsuccessfully' })}！`
      );
  ...
  _checkUploadFile = (file, fileList) => {
    ...
    if (fileList.length > choosedImgCount) {
      if (fileList.filter((f) => f.uid).length == fileList.length) {
        message.error(
          `${RCi18n({
            id: 'Setting.Upload at most once'
          })}${choosedImgCount} ${RCi18n({ id: 'Setting.pictures' })}`
        );
      }
      return false;
    }
  ...
  _handleUploadClick = () => {
    ...
    if (cateId) {
      ...
      message.error(RCi18n({ id: 'Please select a picture category' }));
    }
  };
}
