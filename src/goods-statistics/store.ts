...
export default class AppStore extends Store {
  ...
  changeTableFlag = async (flag: number) => {
    ...
    //将全部的排序指标恢复成默认
    await this.selectIndicatorsOrder(flag, '2', '1');
  ...
  init = async (defaultSkuColumns, defaultCateColumns, defaultBrandColumns) => {
    ...
    //获取当天的商品概况和商品报表
    const { res: goodsGeneral } = await webapi.getGoodsTotal({
      selectType: 0,
      companyId: this.state().get('companyInfoId')
    });
  ...
  getbrandTable = async (type, pageNum, pageSize) => {
    ...
    const { res } = await webapi.getbrandList({
      ...
      id:
        this.state().get('brandId') == '-1' ? '' : this.state().get('brandId'),
      companyId: this.state().get('companyInfoId')
    });
    if (res.code == Const.SUCCESS_CODE) {
      this.transaction(() => {
  ...
  //   this.dispatch('brand:brandName', name)
  ...
  searchByCateName = (key, value) => {
    const dateFlag = this.state().get('dateFlag');
    this.transaction(() => {
      this.dispatch('cate:cateId', key);
      this.dispatch('cate:cateName', value);
    });
    this.getcateTable(
      dateFlag,
      this.state().get('catePageNum'),
   ...
   * 设置companyInfoId
   */
  setCompanyInfoId = (companyId, storeId) => {
    this.dispatch('goods:total:companyInfoId', companyId);
    //切换店铺时,分类和品牌的下拉要刷新
    this.changeCatesBrands(storeId);
   ...
   * @param storeId
   */
  changeCatesBrands = async (storeId) => {
    //storeId为真时
    if (storeId) {
      //查询店铺分类和店铺签约品牌
      const { res: cates } = await webapi.getStoreCates(storeId);
      const { res: brands } = await webapi.getStoreBrands(storeId);
      if (
        cates.code == Const.SUCCESS_CODE &&
        brands.code == Const.SUCCESS_CODE
      ) {
        this.transaction(() => {
          this.dispatch('cate:list:shop', fromJS(cates.context));
          this.dispatch('brand:list', fromJS(brands.context));
        });
      }
    } else {
