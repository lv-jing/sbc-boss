...
export default class AppStore extends Store {
  ...
  init = async (activityId) => {
    const { res } = await webapi.queryActivityEnableTime(3, activityId);
  ...
  save = async () => {
    ...
    } else if (res.code == 'K-080113') {
      info({
        content: res.message,
        okText: '好的'
      });
      const { res: queryTimeRes } = await webapi.queryActivityEnableTime(
        3,
        activity.activityId
      );
      if (queryTimeRes.code == Const.SUCCESS_CODE) {
        const disableTimeList = queryTimeRes.context;
        this.dispatch('set: disable: time', fromJS(disableTimeList));
      }
    } else {
