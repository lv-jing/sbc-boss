...
export default class MoveVideoModal extends React.Component<any, any> {
  ...
  render() {
    ...
    return (
      <Modal maskClosable={false} title={<FormattedMessage id="Setting.MobileVideo" />} visible={moveVisible} onCancel={this._handleCancel} onOk={this._handleOk}>
      ...
      <Form>
        <FormItem {...formItemLayout} label={<FormattedMessage id="Setting.ChooseCategory" />} required={true} hasFeedback>
          {getFieldDecorator('cateId', {
            rules: [{ required: true, message: <FormattedMessage id="Setting.PleaseSelectACategory" /> }]
          })(
            <TreeSelect
              showSearch
