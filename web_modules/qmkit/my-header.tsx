...
export default class NavLayout extends React.PureComponent<any, any> {
  ...
  render() {
    ...
    const email = loginInfo && loginInfo.email;
    ...
    //从缓存中获取二维码地址
    let miniProgramQrcode = localStorage.getItem(cache.MINI_QRCODE);
    ...
    if (baseConfig) {
      if (baseConfig['pcWebsite']) {
        ...
        pcDom = (
          <Menu.Item key="0">
            <a target="_blank" href={baseConfig['pcWebsite']}>
      ...
      if (baseConfig['mobileWebsite']) {
        ...
        h5Dom = (
          <Menu.Item key="1">
            <a target="_blank" href={baseConfig['mobileWebsite']} className="previewMobile">
      ...
      if (miniProgramQrcode) {
        miniProgramDom = (
          <Menu.Item key="2">
            <a target="_blank" href={baseConfig['mobileWebsite']} className="previewCode">
              <img
              ...
              <p>小程序</p>
              <div className="previewImg">
                <img src={miniProgramQrcode} alt="二维码" />
              </div>
            </a>
          </Menu.Item>
        );
    ...
    if (pcDom || h5Dom || miniProgramDom) {
      menuPreview = (
        <Menu className="menuPreview">
          {pcDom ? pcDom:null}
          {h5Dom ? h5Dom:null}
          {miniProgramDom ? miniProgramDom:null}
        </Menu>
      );
    } else {
      menuPreview = (
        <Menu className="menuPreview">
          <Menu.Item key="1">
            <Link to="/basic-setting">请先在基本设置中配置预览链接</Link>
          </Menu.Item>
        </Menu>
      );
    }
        ...
        <div>
          <Dropdown overlay={menu} trigger={['click']}>
            <a className="ant-dropdown-link" href="javascript:;">
              {/* <Icon type="user" /><img src={userImg} alt="" /> */}<span style={styles.dropdownText}>{accountName}</span><Icon type="down" />
            </a>
          </Dropdown>
        </div>
      </Header>
