...
export function validatorRatioLength(_rule, value, callback, _fieldText) {
  ...
  if (
    ...
    callback('Please fill in the correct coefficient');
    return;
  } else {
    let valNumber = Number(value);
    const val = value.toString();
    if (valNumber > 1 || valNumber < 0 || val.trim().length > 4) {
      callback('Please enter a coefficient between 0 and 1, with two decimal places');
      return;
    }
  }
  callback();
...
export function encryptionEmail(email) {
  return email ? email.slice(0,3) + '*****' + email.substr(-3) : '-';
}
